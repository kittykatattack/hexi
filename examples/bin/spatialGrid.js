...
function play() {
  ...
  var spatialGrid = function spatialGrid(widthInPixels, heightInPixels, cellSizeInPixels, spritesArray) {

    //Find out how many cells we need and how long the
    //grid array should be
    var width = widthInPixels / cellSizeInPixels,
    ...
    spritesArray.forEach(function (sprite) {

      //Find out the sprite's current map index position
      var index = getIndex(sprite.x, sprite.y, cellSizeInPixels, cellSizeInPixels, width);
  ...
  var grid = spatialGrid(512, 512, 64, marbles.children);
  ...
  var _loop = function _loop(i) {
    ...
    var sprite = marbles.children[i];

    //Find out the sprite's current map index position
    var gridWidthInTiles = 512 / 64;
    var index = getIndex(sprite.x, sprite.y, 64, 64, gridWidthInTiles);

    //Find out what all the surrounding nodes are, including those that
    //might be beyond the borders of the grid
    var allSurroundingCells = [grid[index - gridWidthInTiles - 1], grid[index - gridWidthInTiles], grid[index - gridWidthInTiles + 1], grid[index - 1], grid[index], grid[index + 1], grid[index + gridWidthInTiles - 1], grid[index + gridWidthInTiles], grid[index + gridWidthInTiles + 1]];

    //Find all the sprites that might be colliding with this current sprite
    for (var j = 0; j < allSurroundingCells.length; j++) {

      //Get a reference to the current surrounding cell
      var cell = allSurroundingCells[j];
    ...
    grid[index] = grid[index].filter(function (x) {
      return x !== sprite;
    });
  };
